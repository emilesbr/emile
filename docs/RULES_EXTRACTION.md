...
**IMPLÉMENTÉ pour Faible/Modéré (24e/25e rounds, cf. `docs/PLAN.md`)** : le changement structurel identifié ci-dessus (fractions de clôture PAR TRANCHE, pas un scalaire par run) a été construit dans `position_engine.py` (24e round), puis câblé sans condition (`backtest_phase2_faithful.py::range_money_management_fracs`, réutilisée par `unified_protocol.py`) : FAIBLE bascule réellement sur `val_close=0,25`/`conf_close=0,50` en régime RANGE_TENDANCIEL (override réel), MODERE ne change RIEN (numériquement identique à §3, vérifié). "Target 1"=TP100% pour ces 2 profils est textuellement identique à "Limite" de §3, aucun nouveau stage nécessaire. Résultat honnête mesuré : quasi nul (51 tranches ouvertes en régime RANGE_TENDANCIEL sur BTC/ETH/BNB/SOL FAIBLE, 1 seule atteint un jour la Validation avant de se fermer, +0,08 pt d'équité sur BTC seul) — implémenté quand même. Détail complet : `docs/PLAN.md` section "25e application".

**Agressif/Très Agressif — "SL gain" câblé au 45e round, décision directe de l'utilisateur d'accepter une lecture extrapolée (H-SLGain-1, `position_engine.py` : stop remonté au plus haut observé depuis l'ouverture, même primitif que Neuneu "SL déplacé au sommet récent") après une demande explicite ("trouve une solution")**. Confirmation littérale câblée sans condition pour TRES_AGRESSIF (conf_close 0,00→0,25, "TP25%+SL BE" — AGRESSIF n'a besoin d'aucun override, déjà identique à §3). "Target 1" (TP25%+SL gain, la seule partie réellement EXTRAPOLÉE) câblée en OPT-IN (`use_sl_gain`, défaut `False`, même statut que `use_mtf_cascade`) — clôture partielle 25% + stop remonté, `position_engine.py::process_tranche`/`make_open_tranche_fn`. **Mesuré honnêtement : mécanisme correct (vérifié par tests synthétiques dédiés qui l'exercent réellement) mais INERTE sur BTC/ETH/BNB/SOL** — `emile/core/sl_gain_wired_measure.py`, 0/8 lignes actif×profil montrent un écart ; cause identifiée, pas seulement constatée : sur les 41 tranches TRES_AGRESSIF ouvertes en régime RANGE_TENDANCIEL, aucune n'atteint même la Confirmation avant de se fermer autrement. Détail complet : `docs/PLAN.md` section "45e application".
