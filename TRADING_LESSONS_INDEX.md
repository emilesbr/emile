...
| 15 | `TRADING_LESSONS_PYRAMIDALISATION.md` | Pyramidalisation (renforcement sur 3ème borne post-breakout, ×5 le rendement), gestion du risque 2%/1:4 | **🎯 6ᵉ confirmation, LA PLUS LITTÉRALE** : "Timeframe d'Exécution... on descend de deux niveaux sous la référence" — correspond quasi mot pour mot à la description initiale | 3ᵉ/4ᵉ confirmation de la règle du breakeven différé (validation N avant BE sur N-1) |

| 16 | `TRADING_LESSONS_CLUSTERS_PRIX.md` | Pattern "Cluster Technique" (MA20+support), diversification 1%+1%, "Règle de Trois", "Extreme Channel" | **🎯 7ᵉ confirmation, tableau quasi identique à #15** | 5ᵉ/6ᵉ confirmation quasi mot-pour-mot du breakeven différé |

## 🎯 Statut de la question initiale (règle multi-timeframe) — RÉSOLUE (confiance maximale, 7 sources convergentes)
**Six formulations convergentes trouvées** (sources #5, #9/#10, #11, #12, #14, #15), toutes de la même famille. La source #15 est la plus littérale : *"Timeframe d'Exécution (Horaire) : on descend de deux niveaux sous la référence (Daily)"* — correspond quasi mot pour mot à la description initiale de l'utilisateur ("pas la sous-jacente mais la deuxième sous-jacente"). Le principe général — la structure/tendance du TF supérieur prime systématiquement, avec des rôles distincts par niveau (référence = tendance de fond, contexte = canaux/respiration à 1 niveau en dessous, exécution = agressivité maximale à 2 niveaux en dessous) — est maintenant considéré comme confirmé avec une confiance élevée. La séquence complète à 7 niveaux (trimestre→15min) évoquée initialement au tout début n'a pas été retrouvée telle quelle — les sources disponibles utilisent systématiquement un exemple à 3 niveaux (ex: Daily/4h/H1), probablement le principe général illustré à chaque fois avec un exemple différent selon le marché/contexte de la vidéo.
...
| 17 | `TRADING_LESSONS_DISSONANCE_HPA.md` | Axe HPA, spirale de dégradation 3 phases (burnout clinique), 3 profils identitaires face à tendance adverse | N/A (psychologie pure) | ⚠️ Avertissement clinique sérieux sur le burnout — Phase 3 = arrêt total 6-12 mois + suivi médical |
