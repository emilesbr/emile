...
| **Diversification 1%+1% (2 patterns indépendants) + "Cluster Technique" (MA20+support) comme second pattern** | #16 | Scopé hors périmètre dès le départ, jamais repris depuis. Chantier tenté ce cycle (agent délégué) mais interrompu par un rate-limit d'infrastructure avant tout code écrit — reste entièrement à faire |
| **Mécanisme "+Reverse" du profil Très Agressif à la Limite — TABLE RANGE spécifiquement** (`position_engine.py`) | Extraction manuel, §3 | Toujours jamais construit pour la table RANGE (`position_engine.py` reste structurellement long-only). À ne pas confondre avec le "+Reverse" de la table TENDANCE (§4, profil Très Agressif, étape Excès final) : celui-là est désormais construit, dans une version simplifiée, cf. note "Table trade de tendance" ci-dessus (hypothèse H10 de `code/trend_table.py`). Chantier tenté ce cycle (agent délégué) mais interrompu par un rate-limit d'infrastructure avant tout code écrit — `position_engine.py` reste inchangé, reste entièrement à faire |
