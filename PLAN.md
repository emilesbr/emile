...
| 4 | `proxy_v2.py::compute_ascending_lows` (structure) utilise `scipy.signal.argrelextrema(..., order=SWING_ORDER)` en un seul appel batch sur toute la série — par construction, la classification "swing low" à l'instant *t* dépend des `SWING_ORDER=3` barres qui suivent *t*, réellement futures dans l'historique complet, pas un artefact de bord. **Vérifié empiriquement sur BTC H4 réel (200 barres échantillonnées) pendant la vérification du travail Fibonacci** : 23/200 (11,5%) changent de classification entre le calcul batch (production) et un calcul recalculé sans aucune barre future. Le même `argrelextrema` batch est aussi utilisé pour `n_borders` (maturité, `backtest_phase2_v7.py::prepare`) et pour les swings de `code/fibonacci.py` (créé ce cycle, réutilise explicitement "la même détection") | Même catégorie que l'occurrence #2 (fonction batch non causale), sur une fonction différente — jamais suspectée avant que la vérification du travail d'un agent tiers (Fibonacci) n'y mène incidemment | **Trouvé ce cycle, PAS encore traité** — ampleur bien plus petite que #2 (fenêtre fixe de 3 barres, pas la série entière), mais présent depuis v4 (maturité par swing points) et désormais aussi dans Fibonacci. À traiter : décaler la classification de `SWING_ORDER` barres avant usage (ou recalcul causal équivalent), puis quantifier l'impact réel sur v4/v5/v6/v7/Fibonacci comme fait pour le P0 |
...
| 8 | **Nouveau, trouvé ce cycle** : `compute_ascending_lows` (structure, `proxy_v2.py`) utilise `argrelextrema` en un seul appel batch — classification à l'instant t dépendante de 3 barres futures réelles, vérifié empiriquement (23/200 barres BTC H4 changent de classification une fois calculé sans barres futures). Même fonction batch réutilisée par `n_borders` (maturité, v4-v7) et par le `code/fibonacci.py` tout juste construit | Affecte la fiabilité du composant "structure" et de la maturité (≥3 bornes) dans TOUS les moteurs depuis v4, plus Fibonacci désormais — ampleur small (fenêtre fixe 3 barres) mais jamais quantifiée avant ce jour, cf. occurrence #4 ci-dessus | Moyen (décaler de `SWING_ORDER` barres avant usage + requantifier v4-v7/Fibonacci, sur le modèle du traitement P0) | **P0-bis — priorité immédiatement après le backlog en cours, avant tout nouvel élément non encore implémenté** |
