...
**Protocole unifié — routeur range ↔ tendance, traité ce cycle** : `code/unified_protocol.py` répond "oui" à la question "les moteurs sont-ils unifiés dans un même protocole ?" (jusqu'ici non, `PLAN.md`/`CONFIGURATION_RECOMMANDEE.md`) — routeur bar-par-bar entre `recommended.py` (RANGE) et `trend_table.py` (TENDANCE), priorité tendance sur déclencheur `accumulation_active`, exclusivité mutuelle par actif. Résultat mesuré honnête (BTC/ETH/BNB/SOL × 4 profils) : 123 campagnes tendance déclenchées mais toutes closes en étape Accumulation (cohérent avec `trend_table.py` seul) — 14/16 combinaisons dégradées en retour vs `recommended.py` seul (moyenne -15,6 pts), pas une amélioration. `decide_now()` fournit une décision live structurée. 14e moteur de `code/run_all.py` (alias `unified`), `code/test_unified_protocol.py` (6/6 tests). Détail : `CONFIGURATION_RECOMMANDEE.md` section 5bis, `PLAN.md` section "Protocole unifié".
