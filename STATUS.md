...
**Protocole unifié — routeur range ↔ tendance, traité ce cycle puis CORRIGÉ** : `code/unified_protocol.py` répond "oui" à la question "les moteurs sont-ils unifiés dans un même protocole ?" (jusqu'ici non, `PLAN.md`/`CONFIGURATION_RECOMMANDEE.md`) — routeur bar-par-bar entre `recommended.py` (RANGE) et `trend_table.py` (TENDANCE), **gérés de façon INDÉPENDANTE** (l'exclusivité mutuelle par actif de la version initiale a été retirée après contestation directe de l'utilisateur, non justifiée par le corpus — au contraire contredite par les sources #15/#16 sur les positions multiples/diversification simultanée, cf. "CORRECTION" dans `unified_protocol.py`). Résultat mesuré honnête (BTC/ETH/BNB/SOL × 4 profils, regénéré après la correction) : 147 campagnes tendance déclenchées (davantage qu'avec l'exclusivité) mais toutes closes en étape Accumulation (cohérent avec `trend_table.py` seul) — 10/16 combinaisons dégradées en retour vs `recommended.py` seul (moyenne -19,2 pts), toujours pas une amélioration. `decide_now()` fournit une décision live structurée, RANGE et TENDANCE rapportés séparément. 14e moteur de `code/run_all.py` (alias `unified`), `code/test_unified_protocol.py` (6/6 tests, révisés pour la concurrence). Détail : `CONFIGURATION_RECOMMANDEE.md` section 5bis, `PLAN.md` section "Protocole unifié".
...
| `code/unified_protocol.py`, `code/test_unified_protocol.py` | Protocole unifié (routeur bar-par-bar RANGE ↔ TENDANCE, `run_unified`/`decide_now`) — cf. `CONFIGURATION_RECOMMANDEE.md` section 5bis, `PLAN.md` section "Protocole unifié". 14e moteur de `code/run_all.py` (alias `unified`), 6/6 tests |
